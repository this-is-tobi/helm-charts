...
### Basic Deployment (Operator-Managed Credentials - Recommended)

Deploy a basic 3-instance PostgreSQL cluster. The CNPG operator auto-generates secure, stable passwords:

```bash
# Deploy cluster (passwords managed by the operator, stable across helm upgrades)
helm install my-postgres oci://ghcr.io/this-is-tobi/helm-charts/cnpg-cluster

# Retrieve the generated application password
kubectl get secret my-postgres-cnpg-cluster-app -o jsonpath='{.data.password}' | base64 -d
```

This creates:
- A 3-instance PostgreSQL cluster with automatic failover
- Operator-managed secure passwords (stable across `helm upgrade` and ArgoCD syncs)
- An application user and database named after the release
- Services for read-write (`my-postgres-cnpg-cluster-rw`) and read-only (`my-postgres-cnpg-cluster-ro`) connections
...
```yaml
# Minimal configuration with operator-managed passwords (recommended)
dbName: myapp
credentials:
  username: myapp_user
  # password and postgresPassword managed by the CNPG operator
```

## Credentials Management

The chart supports three credential management modes, **auto-detected** from the values you provide:

### 1. Operator-managed (default — recommended)

When `credentials.password`, `credentials.postgresPassword`, and `credentials.existingSecrets.enabled` are all left empty/false, the chart does **not** create any credential secrets. Instead, the CNPG operator auto-generates `<cluster>-app` and `<cluster>-superuser` secrets with secure random passwords.

**Why this is recommended:**
- Passwords are **stable across `helm upgrade`** and ArgoCD syncs — no rotation unless you explicitly trigger it.
- Works seamlessly with GitOps tools (ArgoCD, Flux) that use `helm template` without cluster access.
- No secrets in your values files or Git history.

```yaml
credentials:
  username: myapp
  # password: ""           — leave empty (default)
  # postgresPassword: ""   — leave empty (default)
```

To retrieve the generated password:
```bash
kubectl get secret <release>-<chart>-app -o jsonpath='{.data.password}' | base64 -d
```

### 2. Chart-managed (explicit passwords)

When you set `credentials.password` and/or `credentials.postgresPassword`, the chart creates `<cluster>-admin` and `<cluster>-app` secrets. This also enables the `infosSecret` feature (connection string secret).

For any password not explicitly set, the chart uses Helm's `lookup` function to read the existing secret from the cluster — preserving the password across `helm upgrade`. If the secret doesn't exist yet (first install), a random 24-character password is generated.

> **⚠️ GitOps warning (ArgoCD, Flux):** These tools render templates without cluster access, so `lookup` always returns empty. If you use chart-managed mode with a GitOps tool, you **must** set both `credentials.password` and `credentials.postgresPassword` explicitly — otherwise a new random password will be generated on every sync.

```yaml
credentials:
  username: myapp
  password: "my-known-password"
  postgresPassword: "my-admin-password"

infosSecret:
  create: true  # Only works in chart-managed mode
```

### 3. External secrets (pre-existing)

When `credentials.existingSecrets.enabled: true`, the chart references your pre-created Kubernetes secrets. Use this with External Secrets Operator, Vault, Sealed Secrets, or manual `kubectl create secret`.
...
credentials:
  ...
  existingSecrets:
    enabled: true
    app:
      secretName: "my-app-credentials"       # type: kubernetes.io/basic-auth
    postgres:
      secretName: "my-admin-credentials"     # type: kubernetes.io/basic-auth
```

### Mode comparison

| Feature | Operator-managed | Chart-managed | External |
|---------|:---:|:---:|:---:|
| Passwords stable on upgrade | ✅ | ✅ (`lookup` or explicit) | ✅ |
| ArgoCD/GitOps compatible | ✅ | ⚠️ (set both passwords explicitly) | ✅ |
| No secrets in values/Git | ✅ | ❌ | ✅ |
| `infosSecret` support | ❌ | ✅ | ❌ |
| Requires pre-created secrets | ❌ | ❌ | ✅ |
...
> - **Operator-managed passwords** (default, recommended): Leave credentials empty — the CNPG operator generates stable secrets automatically
...
credentials:
  username: dev_user
  # Passwords managed by the operator
...
**Using operator-managed credentials** (recommended — use the operator-generated `<cluster>-app` secret directly):
...
credentials:
  username: myapp
  # password managed by the operator
    ...
    spec:
      ...
      - name: app
        ...
        - name: DB_PASSWORD
          valueFrom:
            secretKeyRef:
              name: <release>-cnpg-cluster-app  # Operator-generated secret
              key: password
        - name: DB_USER
          valueFrom:
            secretKeyRef:
              name: <release>-cnpg-cluster-app
              key: username
```

**Using chart-managed credentials with `infosSecret`** (requires explicit password):
...
> **Note:** `infosSecret` requires chart-managed credentials (explicit `credentials.password`). When using operator-managed or external credentials, reference the operator-generated `<cluster>-app` secret directly.
...
| credentials.password | string | `""` | Password of the database user. When empty and `existingSecrets.enabled` is `false`, the CNPG operator auto-generates a stable secret (recommended). In chart-managed mode (when at least one password is set), empty passwords are resolved via `lookup` to reuse existing secrets — but `lookup` does not work with ArgoCD/GitOps, so set both passwords explicitly in that case. |
| credentials.postgresPassword | string | `""` | Password of the postgres superuser. Same behavior as `credentials.password` — leave empty to let the CNPG operator manage it, or set explicitly for chart-managed mode. |
